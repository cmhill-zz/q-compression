...
their length, it may be possible to identify such common patterns in
...
vector quantization method, which partitions a set of samples into $k$
sets that minimize within-cluster sum of
squares~\citep{macqueen1967some}. We sampled 10K reads at random and
...
optimal minimum~\citep{hartigan1979algorithm}.  The obtained cluster
were then used quality profile prototypes for the dataset.  All reads
are evaluated with the nearest quality profile in Euclidean space
being assigned to every read as their compressed representation.  The
...
No assembly of the \textit{Rhodobacter sphaeroides} 2.4.1 data set was
...
It is important to highlight that the downstream applications dictate
which lossy compression methods are acceptable. As highlighted
earlier, 2-bin encoding is able to retain information about low
