...
values. Furthermore, although lossless representation is necessary for
nucleotide sequences, it is not an essential requirement for quality
values.

Existing methods for compressing quality values mostly focus on
...
better storage efficiency, and assess the resulting impacts on common
bioinformatic analyses using sequence read data.

\section{Results:}
Lossy compression of quality information can greatly decrease storage
and memory requirements with little negative effect on subsequent
analysis results. The three compression strategies in this study were
able to produce results similar to those obtained with the original
uncompressed quality values when performing quality control, genome
assembly, and alignment of short reads to a reference sequence.
...
genomics, are characterized by having relatively few insertion and
...
from compression on subsequent genomic analyses, including read
preprocessing (filtering and trimming), genome assembly, and read
mapping.
...
need to be made. We adopt similar ``good''/``bad'' quality value assignments as the preprocessing tool Sickle~\citep{sickle}, which are 40 and 10 respectively.
...
(0.099 bits/base-call) has a \textsc{mse} of 17.29, whereas 256-profile
encoding only requires 0.079 bits/base-call and has a \textsc{mse} of 11.85.
...
performs quite well in all use cases, and has the best performance in
terms of bits/base-call versus error.
...
results in subsequent analyses, and in some cases surpass performance
