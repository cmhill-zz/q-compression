...
sequences. Furthermore, although lossless representation is necessary
for nucleotide sequences, it is not an essential requirement for
quality values.
...
1,145,8940 paired-end reads; read length 300 nt). The corresponding
...
consisted of 18,828,274 reads of length 36 nt.
...
\caption{The relationship of bits/base-call and mean squared error for
  quality value compression methods and subsequently further
  compressed using a general algorithm via \textsc{bz}ip2 to four data
  sets: \textit{Rhodobacter sphaeroides} 2.4.1; \textit{Homo sapiens}
...
marginal costs, and subsequently further compressed using a general
algorithm via \textsc{bz}ip2 (Figure \ref{fig:mse_vs_bpbp}). Here we
present the fragment libraries for the \textit{Rhodobacter
  sphaeroides}, and \textit{Homo sapiens} data sets; the short-jump
library results are available in Supplemental Table 1. Storing the
uncompressed quality values requires 1 byte/base-call because they are
stored in \textsc{ascii} format, and the lossless compression of each
data set using \textsc{bz}ip2 ranges from 2.19--3.10 bits/base-call;
these values provide a reference point using a widely available
compression program. Values from the same class of compression method
tend to cluster together across the different data sets, with the
0-degree polynomial regression, profile encodings, and
