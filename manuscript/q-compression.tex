...
in genomics research because of great redundancy, inclusion of quality
values, and read-level naming and metadata. Because of this abundance
effective compression of read data has the potential for substantial
improvement in data storage and transfer efficiency.  Among the
...
sequencing technologies. Illumina sequencing reads, the focus of this
...
base-call for Illumina sequencing reads.

Quality values are typically used throughout bioinformatics pipelines.
Software such as \textsc{fastqc}~\cite{citeulike:11583827} and
Prinseq~\cite{Schmieder:2011gd} provide the user with detailed
descriptions of the distribution of sequence quality scores, and these
scores can be used to guide filtering and trimming of sequencing read
data as processing step before subsequent analyses. Quality values can
be used by genome assembly software to produce better
assemblies~\cite[e.g.,][]{Gnerre:2011kx}.  Short-read alignment
software, such as Bowtie 2~\cite{Langmead:2012rw}, use quality values
to scale the mismatch penalty in a way that mismatches between poor
quality bases of the query and the reference are weighted less than
mismatches occurring on high quality bases. Similar to alignment,
software for detecting single nucleotide polymorphisms (\textsc{snp}s)
can use quality values~\cite[e.g.,][]{McKenna:2010bh}, and identified
\textsc{snp}s with high-quality calls are deemed more reliable than
...
We examine three general classes of lossy compression methods:
binning, model-based, and profile-based.
...
Quality values can be binned into a set of bins, and any number of
bins less than the the number of distinct quality values is
possible. The minimum number of bins that allows for a any distinction
among quality values is two, i.e., two categories ``good'' and ``bad''
quality. We implement 2-bin encoding by setting a quality value
threshold empirically determined by the distribution of quality values
across reads. Base-calls are marked ``bad'' if their quality value
falls below the first quartile minus 1.5 $\times$ the interquartile
range (IQR), which is the difference between the first and third
quartile. 1.5 $\times$ IQR is the value used by Tukey's box
...
quality values, but we only analyze 2-bin encoding in our results.
...
provides. Therefore the compression ratio and bits/base-call values
for polynomial regression throughout this manuscript refer to
...
For quality control, we use
Sickle~\cite{sickle}. PrinSeq~\cite{Schmieder:2011gd} was used to
count the number of bases and reads in each \textsc{fastq} file.
...
For aligning the sequences, we use Bowtie2~\cite{Langmead:2012rw} with
default settings and modified max- and min-mismatch parameters
detailed below.
...
we use mean bits/base-call to measure the physical compression of the
data and mean squared error (\textsc{mse}) to measure the loss in
quality. Bits/base-call is calculated simply as the compressed file
size (in bits) divided by the total number of quality
values. \textsc{mse} is calculated as
$\frac{1}{n}\sum_{i=1}^{n}{(Q_i-Q_i')^2}$, where $n$ is the number of
sequences, $Q_i$ is the original quality value associated with
sequence position $i$, and $Q_i'$ is the corresponding compressed
quality value.
...
\textsc{fastq} files. Sickle is a tool to filter and trim reads. We
used PrinSeq to calculate the number of reads and bases in each
sample. Table \ref{tab:quality_control} shows the proportional numbers
with respect to those of the uncompressed \textsc{fastq} files. In the
...
using 2-bin encoding, 0--5th-order polynomial regression encodings,
and 32- and 256-profile encodings. We evaluate the assembly based on
contiguity statistics, log average read probability
...
script provided by \textsc{gage} which counts single nucleotide
...
of the assembly to aid in comparison. Alongside the raw metrics, we
also provide a cummulative sum of the individual ranks of each method
for each metric. A similar approach was utilized by the
\textsc{Assemblathon1} competition~\cite{Earl:2011fv}.

The assembly using the uncompressed reads outperforms all compression
methods in terms of \textsc{lap}, NG50, relocations, translations, and
inversions (Table \ref{tab:assembly_ranks}). Among the compression
methods, the 256- and 32-profile encoding had the highest rank, then
the 4th-order polynomial regression, the 5th-order polynomial
regression, the 2-bin encoding, and lastly, the 3rd through 0-order
polynomials.
...
compression methods there are no explicit fixed costs in terms of
memory, unlike the profile--based approach, which has a small fixed
cost in storing the profiles. However, the profile-based approach has
a much smaller marginal compared to the rest due to the fact that we
are simply storing the profile index for each quality sequence.
...
proxies for read quality value sequences.
...
operations, we used Sickle~\cite{sickle} with the default
