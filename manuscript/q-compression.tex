...
\textsc{ascii} format; however, lossless compression of the values with
...
regression has the highest bits per bp,
but have among the lowest \textsc{mse}. As the order of the polynomial increases, the bits per bp increase and
...
\caption{MSE versus bits per bp for different compression methods
  ...
  Polynomial regression-based compression using 0-5 degrees. Dashed line corresponds to lossless
...
\caption{MSE versus bits per bp for for different compression methods
  applied to the short-jump library from \textit{Rhodobacter
    sphaeroides} 2.4.1 data set. G/B: Good/Bad encoding. P32 and P256:
  Quality profile compression with 32 and 256 profiles. R0-R5:
  Polynomial regression-based compression using 0-5 degrees. Dashed line corresponds to
...
and 32- and 256-profile encodings. We evaluate
...
methods, the 
256- and 32-profile encoding had the highest rank, then the 4th-order polynomial
...
inverse relationship with the amount of SNPs detected. The good/bad and
profile methods detected the least amount of SNPs compared to
