...
minimizing the loss of information with less emphasis on the effects on
...
quality sequences when performing quality control, genome assembly, and
...
has the potential to substantially improve data storage and
transfer efficiency.
...
data. At the level of the sequence read, the probability of error for each
base-call is typically represented by a \textsc{phred} quality value,
...
values generally includes two operations: \textit{i}.~filtering, which removes reads
that on the whole do not meet arbitrary quality
...
reliable than those with low-quality calls, particularly in low-coverage regions.
...
as an Illumina HiSeq, the focus of this research due to its ubiquitousness in bioinformatics research, are characterized by
...
categories of quality: ``good'' and ``bad''. We implement 2-bin encoding
...
single bin. LogBinning works similarly to UniBinning, except it uses the
...
a combination of LogBinning and Truncating in that we place the
...
While we focus only on two bins in this work, a greater number of bins can be used in practice. For example, an additional third bin (good, bad, and \emph{okay}) can be used for base qualities near the border between bins in the 2-bin encoding. Examining the trade-off of additional bins and their resulting compressibility and effect on downstream analyses is outside the scope of this work.
...
parameters than a read-length string of raw quality values
...
function of the number of reads times the cost of representing the model
parameters.
...
does approximate the overall patterns in quality values. An example
of 128 quality profiles is shown in Figure \ref{fig:profiles_128}.
...
%While we focus on profiles obtained from $k$-means clustering, other profiling techniques can be used in practice.
We can store the profiles of polynomial functions to increase the compressibility of polynomial regression modeling.
%For example, in order to increase the compressibility of polynomial regression modeling, we can store the profiles of polynomial functions.
...
corresponding reference sequence was obtained from the \textsc{ncbi} RefSeq
...
read to minimize the average error. The quality values can be clustered
beforehand to produce more accurate models.
...
but the short-jump library results are available in Supplemental
Table 1. Storing the uncompressed quality values requires 1
byte/base-call because they are stored in \textsc{ascii} format, which is
...
fragment library, \textsc{q}ual\textsc{c}omp with rate set to 10 bits/read
...
Read-Quality-Sparsifier~\citep{DBLP:conf/recomb/YuYB14} achieved best-case
compression of 0.254 bits/base-call, and a mean compression of
1.841 bits/base-call; both values are higher than all of the profile
...
various regression models. If a poor-quality base is flanked by high-quality sequence, 
then the low-degree polynomial regression models
tend to smooth out the base quality, erroneously marking the low-quality
base as higher quality.  During alignment, Bowtie2 penalizes
mismatches at high-quality bases more so than mismatches at lower-quality
bases. Thus, the polynomial regression models incur a high
penalty for these bases, resulting in fewer alignments, despite having
...
In this paper we have examined several general approaches for lossy compression of sequence quality
values and their effect on subsequent analyses. Although most previous
examinations of lossy compression focused primarily on information
loss and compression effectiveness, we show that common
bioinformatic analyses are robust to substantial loss of information in
quality values.

\section{Availability}
Implementations of the compression methods written in Python and R, and
a pipeline to reproduce the results are available at
