  ...
  tables = c("~/cbcb2/q-compression/results/rhodo3/results/frag_2.fastq.compression",
             #"~/cbcb2/q-compression/results2/staph2/results/frag_1.fastq.compression",
             "~/cbcb2/q-compression//results/ecoli2/results/MiSeq_Ecoli_MG1655_110721_PF_R1.fastq.compression",
             "~/cbcb2/q-compression//results2/hg14_results/frag_1.fastq.compression",
  ...
  #   dfr_tables <- rbind(dfr_tables,dfr)
    ...
    #original_tables <- rbind(original_tables, data.frame(x = original_value$bits_bp, y = original_value$mse))
 ...
 pdf("compression_results.pdf", width=10, height=5)
  ...
  p <- ggplot(dfr_tables, aes(x = x, y = y, label=names, col=dataset, shape=pch, bty="n")) + 
    ...
    #geom_segment(data = original_tables, aes(x = x, y = y+5, xend = x, yend = 0,  arrow = arrow(length = unit(0.5, "cm")))) +
    geom_segment(aes(x = 2.036, y = 5.5, xend = 2.036, yend = 0), arrow = arrow(length = unit(0.2, "cm")), size = 0.25, col=gg_cols[[1]]) +
    #2.795606
    geom_segment(aes(x = 2.825606, y = 5.5, xend = 2.825606, yend = 0), arrow = arrow(length = unit(0.2, "cm")), size = 0.25, col=gg_cols[[2]]) +
    #2.786886
    geom_segment(aes(x = 2.756886, y = 5.5, xend = 2.756886, yend = 0), arrow = arrow(length = unit(0.2, "cm")), size = 0.25, col=gg_cols[[3]]) +
    geom_segment(aes(x = 3.101147, y = 5.5, xend = 3.101147, yend = 0), arrow = arrow(length = unit(0.2, "cm")), size = 0.25, col=gg_cols[[4]]) +
    geom_segment(aes(x = 8, y = 5.5, xend = 8, yend = 0), arrow = arrow(length = unit(0.2, "cm")), size = 0.25, col="black") +
    #geom_segment(data = original_tables, aes(x = x, y = y, xend = 1, yend = 2, arrow = arrow(length = unit(0.2, "cm")))) +
    #geom_segment(aes(x=8, y=6, xend=8, yend=4, arrow = arrow(length = unit(0.5, "cm")), label=c("")),
    #             color="black") +
    xlab("bits/base-call") + 
    ylab("mean squared error") +
    geom_point(size=2) + 
    geom_text(hjust=-.3, vjust=-0.3, size=2, show_guide = FALSE) +  
    theme_bw() + theme(panel.background = element_blank(), legend.position="bottom", legend.key = element_blank(), legend.title=element_blank(),
