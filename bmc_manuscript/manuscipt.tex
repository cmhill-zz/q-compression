...
on lossless compression of base calls~\cite{Deorowicz:2013hq,
Giancarlo:2009fk,Giancarlo:2014rw,
  Nalbantoglu:2010uq,Zhu:2013qr}, although some recent work has
addressed compression of quality
values~\cite{Canovas:2014fr,Hach:2012ys,
...
minus 1.5 $\times$ the interquartile range (\textsc{iqr}), which is the
difference between the first and third quartile; 1.5 $\times$ \textsc{iqr} is
the value used by Tukey's box plot~\cite{mcgill1978variations}. The
...
preprocessing tool Sickle~\cite{sickle}, which correspond to the 40
...
lossy compression tool~\cite{Ochoa:2013rt}. The program models
...
Burrows-Wheeler algorithm~\cite{bwt} via \textsc{bz}ip2, and it is
the results of these subsequent compressions that we report. Although
other algorithms~\cite{HUFFMAN:1952nr,Ziv77auniversal,
...
\section*{Figures}
\begin{figure}[h!]
\centerline{\includegraphics[width=3.35in]{profiles128.eps}}
\caption{\csentence{Quality profiles obtained by $k$-means clustering.} 
  Example for the
  fragment library from \textit{Rhodobacter sphaeroides} 2.4.1 data
  set using $k$ = 128. Each row corresponding to a quality
  ...
  quality. Occasional low values occur early in some profiles, likely
  reflecting intermittent problems in the sequencing process affecting
  many reads at a time.}
  \label{fig:profiles_128}
\end{figure}

\begin{figure*}[h!]
\centerline{\includegraphics[width=7in]{compression.results.eps}}
\caption{\csentence{The relationship of bits/base-call and mean squared error for
  quality value compression methods applied to four data sets.}
  The data sets are \textit{Rhodobacter sphaeroides} 2.4.1; \textit{Homo sapiens}
...
\begin{figure}[h!]
\centerline{\includegraphics[width=3.65in]{preprocessing.results.eps}}
...
\begin{figure}[h!]
\centerline{\includegraphics[width=3.65in]{rhodo.assembly.results.eps}}
\caption{\csentence{Rankings of compression methods based on assembly attributes}
 The read data are for \textit{Rhodobacter
