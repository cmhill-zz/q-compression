...
information loss we use three loss functions: mean squared error
(\textsc{mse}), defined as $\frac{1}{n}\sum_{i=1}^{n}{(Q_i'-Q_i)^2}$;
mean absolute deviation (\textsc{mad}), defined as
$\frac{1}{n}\sum_{i=1}^{n}{|Q_i'-Q_i|}$; and mean Lorentz distortion,
defined as $\frac{1}{n}\sum_{i=1}^{n}{log_{2}(1 + |Q_i'-Q_i|)}$, where
$n$ is the number of quality values, $Q_i'$ is the
